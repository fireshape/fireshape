...
#@pytest.mark.parametrize("inner_t", [#H1InnerProduct,
#                                     #ElasticityInnerProduct,
#                                     LaplaceInnerProduct])
@pytest.mark.parametrize("inner_t", [H1InnerProduct, LaplaceInnerProduct])
def test_periodic(dim, inner_t, pytestconfig):
    ...
    #    ext = ElasticityExtension(Q.V_r)
    #if use_extension == "w_ext_fixed_dim":
    #    ext = ElasticityExtension(Q.V_r, fixed_dims=[0])
    #else:
    #    ext = None
    ext = None

    q = ControlVector(Q, inner, boundary_extension=ext)
