...
class TimeTracking(PDEconstrainedObjective):
    ...
    def __init__(self, *args, **kwargs):
        ...
        perturbation = 0.25*sin(x*pi)*g**2
        ...
        self.F = lambda t, u, u_old: inner((u-u_old)/dt,v)*dx(metadata={"quadrature_degree": 1}) + inner(grad(u), grad(v))*dx(metadata={"quadrature_degree": 1}) - inner(f(t+dt), v)*dx(metadata={"quadrature_degree": 1})

    def solvePDE(self):

        self.J = 0; t = 0
        self.u.assign(self.u0)
        self.J += assemble(inner(self.u - self.u_t(t), self.u-self.u_t(t))*dx(metadata={"quadrature_degree": 2}))
        #self.J += norm(self.u - self.u_t(t))**2
        for ii in range(10):
            self.u_old.assign(self.u)
            solve(self.F(t, self.u, self.u_old) == 0, self.u, bcs=self.bcs)
            t += self.dt
            self.J += assemble(inner(self.u - self.u_t(t), self.u-self.u_t(t))*dx(metadata={"quadrature_degree": 1}))
            #self.J += norm(self.u - self.u_t(t))**2
