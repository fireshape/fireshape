...
mesh = Mesh("mesh/klein.msh", dim = 3)
...
# Initial condition
g = Function(V).interpolate(sin(x[2])*cos(x[1])) #interpolate(Expression("sin(x[2])*cos(x[1])", degree=2), V)

# Body force
bf = Function(V).interpolate(10*sin(10*x[2])*cos(10*x[1])) #interpolate(Expression("sin(x[2])*cos(x[1])", degree=2), V)
...
du= TrialFunction(W)
v = TestFunction(W)
ries = Function(W,name = "RiesBack")
M = assemble(inner(du,v)*dx)
solver = LinearSolver(M)
solver.solve(ries,dJctrl)

print("Solving RIESZ")
File("output/deriv.pvd").write(temp,ries)
